...
void UBFL_CreateBP::GenerateRunWay15LightModelBlueprint_HISM(const FString& FBXPath, const FString& AssetPath, UDataTable* dt)
{
    // Model 一般来说就一个 Mesh
    TArray<FParsedMeshData_FFSLightModel> meshArray = UFBXParserLibrary::ParseLightModelFBX(FBXPath);
        ...
        for (int variant : MeshData.Variants) {
            ...
            if (variant == 2) { 
                ...
                if (dt) {
                    ...
                    if (UStaticMesh* Mesh = LoadStaticMesh_Editor(_data->StaticMeshPath)) {
                        CreateTarModelBlueprint_HISM(packageName, Mesh, MeshData, variant);
                    }
                }
                else {
                    UE_LOG(LogTemp, Warning, TEXT("Not Found DT"));
...
UBlueprint* UBFL_CreateBP::CreateTarModelBlueprint_HISM(const FString& PackageName, UStaticMesh* TarMesh, const FParsedMeshData_FFSLightModel& modelMesh, int TarVariant)
{
#if WITH_EDITOR
